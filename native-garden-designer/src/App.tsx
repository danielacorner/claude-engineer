...
const App: React.FC = () => {
  ...
  const updatePlantPosition = (id: number, newPosition: [number, number, number]) => {
  ...
  const fogColor = useMemo(() => {
    switch (season) {
      case 'Spring': return '#e6f0ff'
      case 'Summer': return '#f0f5ff'
      case 'Autumn': return '#fff0e6'
      case 'Winter': return '#f0f0f0'
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', position: 'relative' }}>
      <Canvas shadows camera={{ position: [0, 5, 10], fov: 50 }}>
        ...
        <pointLight position={sunPosition} intensity={lightIntensity} castShadow />
        ...
        <Suspense fallback={null}>
          {plants.map(plant => (
            <PlantInstance
              key={plant.id}
              plant={plant}
              updatePosition={(newPosition) => updatePlantPosition(plant.id, newPosition)}
